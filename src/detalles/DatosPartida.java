
package detalles;

public class DatosPartida {
    
    String id;
    String folio;
    String hoja;
    String de;
    String modopart;
    String desa;
    String tipo;
    String sello;
    String medida;
    String pig;
    String mat1;
    String mat2;
    String cal1;
    String cal2;
    String cospart;
    String cosmat;
    String kgdes;
    String pordes;
    String puni;
    String pzs;
    String kgs;
    String importe;
    String pzFinales;

    public String getPzFinales() {
        return pzFinales;
    }

    public void setPzFinales(String pzFinales) {
        this.pzFinales = pzFinales;
    }
    
    

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }
    
    
    public DatosPartida(){
        
    }

    public String getFolio() {
        return folio;
    }

    public void setFolio(String folio) {
        this.folio = folio;
    }

    public String getHoja() {
        return hoja;
    }

    public void setHoja(String hoja) {
        this.hoja = hoja;
    }

    public String getDe() {
        return de;
    }

    public void setDe(String de) {
        this.de = de;
    }

    public String getModopart() {
        return modopart;
    }

    public void setModopart(String modopart) {
        this.modopart = modopart;
    }

    public String getDesa() {
        return desa;
    }

    public void setDesa(String desa) {
        this.desa = desa;
    }

    public String getTipo() {
        return tipo;
    }

    public void setTipo(String tipo) {
        this.tipo = tipo;
    }

    public String getSello() {
        return sello;
    }

    public void setSello(String sello) {
        this.sello = sello;
    }

    public String getMedida() {
        return medida;
    }

    public void setMedida(String medida) {
        this.medida = medida;
    }

    public String getPig() {
        return pig;
    }

    public void setPig(String pig) {
        this.pig = pig;
    }

    public String getMat1() {
        return mat1;
    }

    public void setMat1(String mat1) {
        this.mat1 = mat1;
    }

    public String getMat2() {
        return mat2;
    }

    public void setMat2(String mat2) {
        this.mat2 = mat2;
    }

    public String getCal1() {
        return cal1;
    }

    public void setCal1(String cal1) {
        this.cal1 = cal1;
    }

    public String getCal2() {
        return cal2;
    }

    public void setCal2(String cal2) {
        this.cal2 = cal2;
    }

    public String getCospart() {
        return cospart;
    }

    public void setCospart(String cospart) {
        this.cospart = cospart;
    }

    public String getCosmat() {
        return cosmat;
    }

    public void setCosmat(String cosmat) {
        this.cosmat = cosmat;
    }

    public String getKgdes() {
        return kgdes;
    }

    public void setKgdes(String kgdes) {
        this.kgdes = kgdes;
    }

    public String getPordes() {
        return pordes;
    }

    public void setPordes(String pordes) {
        this.pordes = pordes;
    }

    public String getPuni() {
        return puni;
    }

    public void setPuni(String puni) {
        this.puni = puni;
    }

    public String getPzs() {
        return pzs;
    }

    public void setPzs(String pzs) {
        this.pzs = pzs;
    }

    public String getKgs() {
        return kgs;
    }

    public void setKgs(String kgs) {
        this.kgs = kgs;
    }

    public String getImporte() {
        return importe;
    }

    public void setImporte(String importe) {
        this.importe = importe;
    }
   
}
